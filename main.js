document.querySelectorAll('.scroller').forEach((scroller, index) => {
  let baseSpeed = index === 0 ? 1 : 2.0; // First scroller is slower
  let speed = baseSpeed;
  let position = 0;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  gsap.to(popupOverlay, { duration: 0.3, opacity: 1, visibility: "visible", delay: 1});
  gsap.to(popup, { duration: 0.5, scale: 1, opacity: 1, ease: "power3.out", delay: 1});
  
  // Close popup when the cross button is clicked
  closePopupBtn.addEventListener("click", () => {
    gsap.to(popup, { duration: 0.3, scale: 0.8, opacity: 0, ease: "power3.in" });
    gsap.to(popupOverlay, { duration: 0.3, opacity: 0, visibility: "hidden", delay: 0.3 });
  });
});